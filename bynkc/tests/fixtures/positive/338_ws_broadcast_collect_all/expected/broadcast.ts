...
export class Room {
  ...
  async fanout(text: string, deps: {}): Promise<readonly Result<void, string>[]> {
    const __state = await this.loadState();
    return (async () => { const __out: Result<void, string>[] = []; for (const __x of Object.values(__state.conns)) { __out.push(await (async (c: Connection<ServerFrame>) => {
  ...
  async fanoutPar(text: string, deps: {}): Promise<readonly Result<void, string>[]> {
    const __state = await this.loadState();
    return (async () => await Promise.all(Object.values(__state.conns).map((__x) => (async (c: Connection<ServerFrame>) => {
    await (c).send({ text: text });
    const r: Result<void, string> = Ok(undefined);
    return r;
  })(__x))))();
