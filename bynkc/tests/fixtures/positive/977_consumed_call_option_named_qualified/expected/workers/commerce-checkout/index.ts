...
// Worker entry point for context `commerce.checkout`.
      ...
      if (path.startsWith("/_bynk/call/")) {
        ...
        switch (servicePath) {
          case "pay": {
            ...
            if (__contract !== "aaf73c4200082f27") return new Response(JSON.stringify({ kind: "ContractMismatch", service: "pay", expected: "aaf73c4200082f27", actual: __contract }), { status: 409, headers: { "content-type": "application/json" } });
            ...
            const result = await surface.pay(id);
            const body = handlers.serialise_Result_Receipt_Int(result);
