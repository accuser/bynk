...
async function __prop_test_scalar_int_participates_in_arithmetic() {
    const deps = {};
    const __gens = [
...
export async function run(only?: string) {
  ...
  if (want("scalar int participates in arithmetic")) results.push({ name: "scalar int participates in arithmetic", ...(await __prop_test_scalar_int_participates_in_arithmetic()) });
  return results;
}
