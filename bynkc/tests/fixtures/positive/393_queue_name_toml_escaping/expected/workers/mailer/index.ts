      ...
      case "q\nkey = \"injected": {
        for (const msg of batch.messages) {
          try {
            ...
            if (__r.tag === "Err") { console.error("queue q\nkey = \"injected deserialise failed", __r.error); msg.retry(); continue; }
            const result = await surface.queue_outbox_0(__r.value);
            if (result.tag === "Ack") msg.ack();
            else { console.error("queue q\nkey = \"injected retry", result.reason); msg.retry(); }
          } catch (e) {
            console.error("queue q\nkey = \"injected threw", e); msg.retry();
          }
        }
        return;
      }
