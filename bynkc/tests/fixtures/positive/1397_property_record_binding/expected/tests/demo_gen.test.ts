...
async function __prop_test_generated_record_binding_satisfies_its_own_trivial_shape() {
    const deps = {};
    ...
    const __body = async (__vals: any[]) => {
      const [p] = __vals;
      if (!(true)) { throw __bynkExpectFailure("tests/gen.test.bynk:9:14", 486, 490, "expect true"); }
    };
    return await __bynkRunProperty({ seed: __bynkMix(__bynkSeed, 0), cases: 100, gens: __gens, where: __where, body: __body, name: "generated record binding satisfies its own trivial shape", location: "tests/gen.test.bynk", file: "tests/gen.test.bynk" });
}

export async function run(only?: string) {
  const results = [];
  const want = (n: string): boolean => only === undefined || only === n;
  if (want("generated record binding satisfies its own trivial shape")) results.push({ name: "generated record binding satisfies its own trivial shape", ...(await __prop_test_generated_record_binding_satisfies_its_own_trivial_shape()) });
  return results;
}
