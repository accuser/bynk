...
async function __prop_test_for_all_body_matches_a_locally_constructed_sum() {
    const deps = {};
    ...
    const __body = async (__vals: any[]) => {
      ...
      void (((__d) => {
          switch (__d.tag) {
            case "Hit": {
              const score = __d.score;
              return __bynkExpect((score === 5), "tests/gen.test.bynk:15:30", 780, 790, "expect score == 5");
            }
            case "Miss": {
...
export async function run(only?: string) {
  ...
  if (want("for-all body matches a locally constructed sum")) results.push({ name: "for-all body matches a locally constructed sum", ...(await __prop_test_for_all_body_matches_a_locally_constructed_sum()) });
  return results;
}
