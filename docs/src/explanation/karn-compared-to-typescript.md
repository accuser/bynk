...
- **Refinement is in the type system.** TypeScript can model a `number`, but "a
  ...
  is a [refined type](type-system-philosophy.md): the invalid value cannot be
...
- **Opacity is enforced, not conventional.** TypeScript's branding tricks are
  opt-in and bypassable. Karn's [opaque types](type-system-philosophy.md) are a
  language feature with enforced construction and boundaries.
- **No exceptions, no `null`.** Failure and absence are
  [values](type-system-philosophy.md) (`Result`, `Option`) the caller must
...
- **Architecture is in the language.** Contexts, services, agents, capabilities,
  ...
  [How a Karn program is shaped](how-a-karn-program-is-shaped.md).
- **State and deployment are modelled.** A stateful [agent](the-agent-model.md)
...
- [Why Karn exists](why-karn-exists.md) · [Why compile to TypeScript](why-compile-to-typescript.md)
