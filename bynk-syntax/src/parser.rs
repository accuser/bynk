...
struct Parser<'a> {
    ...
    /// [`TriviaTable`].
    trivia: TriviaTable,
    /// Live recursion depth of the two self-recursive parse entry points
    /// (`parse_expr`, `parse_type_ref`). Incremented on entry and decremented
    /// on exit by [`Parser::enter_recursion`] so it tracks the current stack
    /// depth; when it exceeds [`crate::MAX_NESTING_DEPTH`] the parser reports a
    /// bounded-depth diagnostic instead of overflowing its stack (#713).
    depth: usize,
}
