...
//! kind grounds them at the hover site). A drift test
//! (`store_op_registry_pins_dispatch`) drives every listed operation through
//! the real checker on a `store` field of the matching kind and asserts none is
//! rejected as `unknown_op`, so the table can't list a phantom operation.
...
/// every operation but `remove` additionally requires `given Clock` (eviction
/// reads the clock).
pub const CACHE_STORE_OPS: &[StoreOp] = MAP_STORE_OPS;
...
mod tests {
    ...
    fn ops_for_maps_the_catalogue_and_ignores_unknown_heads() {
        ...
        // `Cache` shares the storage `Map`'s operation set.
        assert_eq!(ops_for("Cache").len(), ops_for("Map").len());
    ...
    fn signatures_lead_with_their_operation_name() {
        for ops in [MAP_STORE_OPS, SET_STORE_OPS, CELL_STORE_OPS, LOG_STORE_OPS] {
            assert!(!ops.is_empty());
            for o in ops {
