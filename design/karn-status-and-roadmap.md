...
repo — compiler (`karnc`), driver (`karn`), formatter (`karn-fmt`), language
server (`karn-lsp`), tree-sitter grammar, and the VS Code extension — assessed
...
Karn is described by three tiers of documents; conflating them produces a
...
2. **The design notes** (`karn-design-notes.md`) and **type-system spec**
   (`karn-type-system.md`) describe an aspirational **v1** language — events,
   ...
   scored as "missing".
3. **The tooling specs** (`karn-lsp-spec.md`, `karn-tree-sitter-spec.md`) and the
   forward roadmaps (`karn-tooling-roadmap.md`, `karn-engineering-roadmap.md`)
   sit alongside.
...
> matrix (ubuntu/macOS/windows, `KARN_REQUIRE_TSC=1`) is the live gate.
...
|---|---|---|
| **Compiler `karnc`** (~42k LOC) | Feature-complete for v0–v0.54 | Whole language wired end-to-end; ~216 positive + ~40 negative fixtures; `tsc --strict` verifies every project fixture's emitted TypeScript. |
| **Driver `karn`** (v0.46) | Complete | Thin orchestrator over `karnc` (override → PATH → sibling resolution); `karn doctor` environment check with a pinned output/exit contract (ADRs 0083–0084). |
...
| **`karn-fmt`** | Strong | Full formatter contract incl. comment preservation; idempotent, round-trip-tested over the corpus. |
| **`karn-lsp`** | Rich | Diagnostics, hover, definition, completion, signature help, inlay hints, semantic tokens, codeLens, call hierarchy, implementation nav, folding/selection, workspace symbols, rename/references (v0.24–v0.43). Remaining: the completion tail + B-1/B-2 polish — tracked in [`tracks/lsp.md`](tracks/lsp.md). |
| **`tree-sitter-karn`** | Lags the language | Strong v0–v0.5 grammar + highlights; behind on newer surface (`on http`/`from <protocol>`, `assert`-expr, `test`/`mocks`, `HttpResult`, actors). See [`karn-engineering-roadmap.md`](karn-engineering-roadmap.md). |
| **`vscode-karn`** | Solid client | LSP client + status bar + scaffolds/walkthrough (v0.38); now bundles the server (B-0). Highlighting is TextMate, not the tree-sitter grammar. |
...
(`karnc/src/lib.rs`) — plus a two-pass multi-file project driver
(`karnc/src/project.rs`, now decomposed into `project/{paths,discovery,
...
- **Collections** (v0.20b, ADRs 0034–0039): built-in immutable `List` and `Map`
  with a thin kernel (`fold`/`foldEff`, `prepend`) and a Karn-written combinator
...
- **Quality gate**: every project-form fixture's emitted TypeScript is compiled
  under `tsc --strict --noEmit` (`karnc/tests/tsc_verify.rs`); an
...
- **Spec/impl primitive divergence.** `karn-type-system.md` §1.1 lists
...
[`karn-tooling-roadmap.md`](karn-tooling-roadmap.md) for the forward plan and
[`tracks/lsp.md`](tracks/lsp.md) for the live slice decomposition.

- **`karn-fmt`** — full formatter contract incl. the hard comment-preservation
  requirement; idempotent and round-trip-tested. Remaining gap: comments buried
  inside expression sub-trees.
- **`karn-lsp`** — the A/B-tier arc shipped across v0.24–v0.43: project
  ...
  and B-1/B-2 polish.
- **`tree-sitter-karn`** — the biggest tooling lag: a strong v0–v0.5 grammar that
  ...
  engineering roadmap.
- **`vscode-karn`** — LSP client, status bar, and B-2 polish (scaffolds,
...
- **Editor tooling** — [`karn-tooling-roadmap.md`](karn-tooling-roadmap.md)
  (LSP + VS Code) and its live track [`tracks/lsp.md`](tracks/lsp.md).
- **Engineering** — [`karn-engineering-roadmap.md`](karn-engineering-roadmap.md):
...
1. Add the implementation-status banner to `karn-type-system.md` (Float vs
   Decimal; which primitives ship).
2. Resolve the `Int`-precision and workers-edge `any` issues before Karn handles
   large integers or where boundary type-safety is load-bearing.
3. Bring `tree-sitter-karn` up to the current surface (see engineering roadmap).
...
Karn is a mature, end-to-end compiler that has executed its entire planned
