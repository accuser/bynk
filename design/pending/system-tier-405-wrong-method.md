...
- **D2 — the router's `405` fall-through is `Rejected(MethodNotAllowed)`, superseding DECISION C.** The `405` joins the `400`/`401` rejections rather than the `HttpResult` vocabulary: the router's method fall-through means *no handler ran* (the response is synthesised before dispatch), which is the definition of `Rejected`. A generic per-service **no-handler driver** (`__sysdrive_wrongmethod_<svc>(method, path)`, test-output only) sends the arbitrary `(method, path)` and decodes through `responseToHttpOutcome`. But `HttpResult.MethodNotAllowed` is *also* a handler-returnable variant, so the decoder classifies on **shape, not status** (the same care as the `400` `kind`-body path): only a `405` carrying an `Allow` header — the router's fall-through (`workers_entry.rs`) — is `Rejected`; a bodyless handler-returned `405` has no `Allow` and stays `Handled`, so a `Wire` call whose handler returns `MethodNotAllowed` still decodes as `Handled(MethodNotAllowed)`. The lowering routes a call whose `(method, path)` is not a declared route — carried into the emitter as a new `system_http_routes` set — to the driver. `expect r is Rejected(MethodNotAllowed)` discriminates the inner kind via the nested-`is` from ADR 0211 (#705).
...
- A wrong-method call reaches no handler, so it takes **no arguments** (extra args → `bynk.test.service_call_arity`, not silently dropped) and **no `by` clause** (a principal → `bynk.test.principal_on_wrong_method`; negative fixture 390). This keeps the check's acceptance identical to what the driver actually forwards (method + path only).
