...
| 5 | [`semantics-in-the-checker.md`](tracks/semantics-in-the-checker.md) | **Settled — Slicing on merge** (settled 8 August 2026; spine [#1126](https://github.com/accuser/bynk/issues/1126), settling PR [#1127](https://github.com/accuser/bynk/pull/1127)). All five design questions closed; the seven `bynk-check/src/analysis.rs`-named diagnostic categories still in `bynk-emit` relocate by priority — five close a named, fixture-pinned editor regression, two close for R3.5 compliance alone with no observable change; `check_function_type_boundaries`'s reach-back hook closes; R10.1 closes with a crate-doc correction, not a `bynk-driver` split; R4.6/R4.11/R10.4 stay in scope narrowed to verify-only |
| 6 | *(to be opened)* | — |
| 7 | *(to be opened)* | — |
| 8 | *(to be opened)* | — |
