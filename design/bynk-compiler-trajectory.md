...
| 7 | [`the-typescript-tree.md`](tracks/the-typescript-tree.md) | **Settled — Slicing on merge** (spine [#1293](https://github.com/accuser/bynk/issues/1293)). All five design questions closed under review; two changed shape in the direction of more evidence — `TsType::Any` elimination is achievable in full with a small, named residual (not the re-opened-phase-6 risk first framed), and R8.1–R8.22 splits three ways (12 already closed, 5 close as a byproduct of this track's own conversion, 2 need named slices) rather than the assumed closed/open binary. 37 slices named across four arcs; none shipped yet |
| 8 | *(to be opened)* | — |

A phase's track opens when the previous phase's probe reads zero. Not before — an open track for a
phase whose prerequisite is unmet is how a trajectory becomes a wish list. Phase 6 is the one
...
next paragraph's own discipline exists to prevent, not caution.
