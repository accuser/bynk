...
| **T0.2′** | *Adoption*, not construction. The three assertion granularities exist and have nine users against 419 category-string fixtures. Convert the fixtures where attribution or a targeted property is the thing under test — starting with the `Roots::Split` fixtures ADR 0198 named as unobservable, three as of #1008, not the fourteen-version-stale thirteen this row previously cited | R11.2 | medium |
...
| **T1.8** | `NonEmpty` → `MinLength(1)` shipped inside `canon_predicate` (#1021) — a zero-signature-change, single-fixture-blast-radius fold. `Positive` → `InRange(1, ∞)` and `NonNegative` → `InRange(0, ∞)` remain **open**: both need a `base: BaseType` threaded through `canon_predicate`/`canon_refinement`/`refinements_match`, and `Positive`/`NonNegative` on `Float` has a real exclusive-vs-inclusive-bound ambiguity with no clean answer today (see #1021's Decision A) | R12.2 | small |
