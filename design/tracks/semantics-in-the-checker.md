...
| **P5.3** ✅ | `schema_registry::reconcile` (+ helpers `snapshot`/`canon_type`/`Reconciled`/`reconcile_one`/`mismatch_error`/`non_additive_error`) relocated to a new `bynk-check::schema_registry` module; `check_platform_lock` (+ `lock_violation`/`LockViolation`) relocated to `bynk-check::project_model` as `phase_platform_lock`, on a from-scratch pure `native_platforms_of_context`/`collect_given_closure` resolution walk rather than `bynk-emit`'s own helper of that name (which builds a TypeScript instantiation expression as it walks — an emission concern `bynk-check` must not depend on, since `bynk-emit` depends on `bynk-check`, never the reverse); both wired into `analyse_project` at the same relative points `run_checks` calls them; closes categories 1 and 5 (no editor-visible change — see §3.2). `emit_diagnostics` moved `bynk-emit` from 30/34 to 25/27 (true/naive) — the 5 codes these two checks own (`bynk.event.schema_version_mismatch`, `bynk.event.non_additive_schema_change`, `bynk.target.vendor_required`, `bynk.target.vendor_conflict`, `bynk.target.browser_bundle_only`), plus `bynk-emit/src/project/schema_registry.rs` dropping its `bynk_syntax::ast` import (`ast_importers` 11 → 10) now that `reconcile` and its `EventDecl`/`TypeRef` usage moved out. `bynk-lsp/tests/analysis_residual_gap.rs`'s `platform_lock_diagnostic_stays_absent` needed no behaviour change (still absent, for the same hardcoded-Cloudflare/Bundle reason as before); no equivalent pin ever existed for schema-registry reconciliation | R3.5, adjacent to R3.11 | P5.0–P5.2 landed (lower priority, per §3.2) |
| **P5.4** ✅ | `process_tests`/`process_integration_tests` split at the check/emit boundary rather than ported whole, per `analysis.rs`'s own note that the emission-coupled types (`CompiledFile`, `RunnableTest`, `ImportExt`, `contracts`) made a single move "out of proportion": the checking half (target/participant resolution, duplicate-case-name detection, `stub`-clause resolution, case/property body type-checking — `ResolvedStub`, `resolve_stubs`, `check_integration_case_body`, `check_test_bodies`, `check_test_case_body`, `check_restated_contract`, `check_property_body`, `check_history_binding`, and their private helpers) relocated to a new `bynk-check::test_suites` module as `phase_test_bodies`/`phase_integration_bodies`, wired into `analyse_project` right after the per-unit compose/check loop (unconditionally, unlike categories 2-6 — mirrors `run_checks`'s own unconditional call); TypeScript emission (Phase 5) stays in `bynk-emit::project::tests_emit`, whose `process_tests`/`process_integration_tests` keep their exact signatures and now call the relocated checking phase instead of duplicating it. Several functions the checking phase needs are genuinely dual-use — also needed to lower a checked body to TypeScript (`build_privileged_resolved`, `typecheck_case_body`, `register_call_record_types`, `check_history_binding`, `history_handlers`, `history_variant_name`, `prop_binding_generable`, `infer_participants`, `PROP_GEN_DEPTH`, and — found during the move, not in the original slice plan — `value_block`) — these stayed `pub` in `bynk-check::test_suites` and `bynk-emit` calls them qualified, rather than duplicating them (the exact drift §9 warns against). Closes category 7, including the go-to-definition/`RefSink` regression. `emit_diagnostics` moved `bynk-emit` from 25/27 to **6/8** (true/naive) — the 19 codes these checks own (`bynk.suite.*`, `bynk.stub.*`, `bynk.tier.*`, `bynk.test.*`, `bynk.contract.*`, `bynk.history.*`, `bynk.val.*`, `bynk.property.*`), exactly the count §1 estimated. `bynk-lsp/tests/analysis_residual_gap.rs`'s `test_body_diagnostics_and_index_bindings_go_missing` flipped to positive coverage (both its absence and its zero-ref-count assertions invert); `bynk-check/tests/differential_analysis.rs`'s `new_entry_point_omits_test_body_diagnostics` tightened from a divergence pin to a parity assertion, per its own doc comment's instruction, and kept (not deleted — its file's own module doc says it self-deletes only once phase 5 removes `run_checks`'s `Mode::Analyse` arm entirely, P5.5+) | R3.5 | P5.0–P5.3 (last, by design — the one category `analysis.rs` itself flagged as needing more care) |
...
before the relocation — see the P5.2 row above), **25/27** after P5.3, **6/8** after P5.4.
...
**Resolved at P5.4, landed.** Neither option this risk named was needed — the emission-coupled types stayed
in `bynk-emit` untouched, because they were never load-bearing for the *checking* half. Splitting at the
check/emit boundary (the option this risk didn't anticipate) worked cleanly: `bynk-check::test_suites` took
every check-only and dual-use function, `bynk-emit::project::tests_emit` kept `CompiledFile`/`RunnableTest`/
`ImportExt`/`contracts` and every TypeScript-codegen function, and the one genuinely dual-use surprise
(`value_block`, not named in the original slice plan) resolved the same way every other dual-use function
did — `pub` in `bynk-check`, called qualified from `bynk-emit` — rather than forcing a new design choice.
