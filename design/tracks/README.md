...
| [`increment-allocation.md`](increment-allocation.md) | [#685](https://github.com/accuser/bynk/issues/685) | Slicing (Slices 0–1 shipped) | Defer the version and ADR-number assignment to a per-merge stamp on `main`, so parallel increments stop conflicting on the two serial counters |
