...
**Arc C — conversion, smallest blast radius first (~22-26 slices, revised again by #1333's own
step (1) closure — see the design pass and "Fourth correction" below; was ~19 pre-Arc-C, ~12-18-
more-from-here right after the design pass settled the `lower.rs` question, honestly no longer
close to the original ~19)**
...
decomposition order, see "Fourth correction" below) → the ICU-formatting cluster (step (11), not
yet proposed) → `emit_integration_module` + `emit_test_module` (unblocked by #1331, not yet
proposed) → `emit.rs`'s own remaining 72-function tree, leaf-to-root per #1331's own named order
...
**Revised estimate, corrected three times now — by review of #1332 (the arithmetic), by #1333's
own step (1) closure (the real per-step sizing), and now by #1335's own step (2) split.** Step (2)
is NOT closed the way step (1) was — #1335 landed 2 of its 6 functions, but the remaining 4 (the
dispatcher + 3 object-builders) genuinely need their own follow-up slice, gated on the
`TsObjectEntry::Raw` decision — so step (2) costs **2 real slices, not the 1 originally budgeted**
(the same shape step (1)'s own correction took, there via a newly-numbered step (11); here, the
remainder stays step (2)'s own identity since it's not a distinct "too big for a leaf helper"
cluster, just a blocked one). Summing the list directly: steps (3)(4)(5)(6)(8)(10) are one slice
each = 6; step (2) is **1** (its own real remainder — #1335 already landed, not double-counted
here); step (7) is "1-2"; step (9) is "3-5"; step (11) (the ICU cluster) is "1-2". Floor:
6 + 1 + 1 + 3 + 1 = **12**; ceiling: 6 + 1 + 2 + 5 + 2 = **16** — `emit.rs`'s own remaining tree is
still **roughly 12-16 slices** (the same range as post-#1333, since step (2)'s own extra slice was
already implicitly present in that range's own step (2) = 1 term — what changed is WHICH slice that
term now refers to: the follow-up, not #1335 itself). Total remaining from here: `emit.rs` (12-16)
+ the `tests_emit.rs` pair (2) — #1331/#1332, slice 8 (#1333), and slice 9 (#1335) are all already
landed, no longer "remaining" — **roughly 14-18 more slices from here**. Arc C's own real total
(the **9** slices already landed — slice 1, the schedule-correction, slices 3/4/5/6/7/8/9 — plus the
14-18 above): **roughly 23-27**, up by 1 from the prior 22-26 (one more slice landed, but step (2)'s
own real cost — 2 slices, not 1 — wasn't reflected in that prior estimate at all) — still **honestly
no longer close to the original pre-Arc-C ~19 estimate**, stated plainly, not softened, the same
discipline every correction in this section has applied. `emit_agent`'s own sub-decomposition
(step 9) remains the single largest source of variance in this range; the ICU cluster (step 11),
step (2)'s own follow-up, and step (7) are smaller, secondary sources.
...
Provisionally lettered, not numbered — Arc C's own slice count is an estimate (~22-26, revised by
#1331 then #1333 — see §6's own "Third"/"Fourth correction" above), so fixed `P7.N` numbers here would silently claim
