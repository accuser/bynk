...
step (2) split and #1337's own step (3) closure — see the "Revised estimate" paragraph below; was
...
`emit_record_type`/`emit_sum_type` trio — see the table row below) → the ICU-formatting cluster
...
**Step (3) is closed by #1337 — the method-emission cluster `emit_attached_methods`/
...
`TsObjectEntry::Raw`-style opaque-entries carrier needed after all; (4) `emit_free_fn`/
...
**Revised estimate, corrected four times now — by review of #1332 (the arithmetic), by #1333's
own step (1) closure (the real per-step sizing), by #1335's own step (2) split, and now by #1337's
own step (3) closure.** Step (2) is NOT closed the way step (1) was — #1335 landed 2 of its 6
functions, but the remaining 4 (the dispatcher + 3 object-builders) genuinely need their own
follow-up slice — no longer gated on the `TsObjectEntry::Raw` decision (#1337 obviated it, see
above), but still a real, separate slice — so step (2) costs **2 real slices, not the 1 originally
budgeted** (the same shape step (1)'s own correction took, there via a newly-numbered step (11);
here, the remainder stays step (2)'s own identity since it's not a distinct "too big for a leaf
helper" cluster, just a blocked-then-unblocked one). Step (3) is now **fully landed** (#1337) and
drops out of the remaining-work sum entirely. Summing the list directly: steps (4)(5)(6)(8)(10)
are one slice each = 5 (step (3) removed, now landed); step (2) is **1** (its own real remainder —
#1335 already landed, not double-counted here); step (7) is "1-2"; step (9) is "3-5"; step (11)
(the ICU cluster) is "1-2". Floor: 5 + 1 + 1 + 3 + 1 = **11**; ceiling: 5 + 1 + 2 + 5 + 2 = **15**
— `emit.rs`'s own remaining tree is now **roughly 11-15 slices** (down from 12-16, since step (3)
is done). Total remaining from here: `emit.rs` (11-15) + the `tests_emit.rs` pair (2) — #1331/
#1332, slice 8 (#1333), slice 9 (#1335), and slice 10 (#1337) are all already landed, no longer
"remaining" — **roughly 13-17 more slices from here**. Arc C's own real total (the **10** slices
already landed — slice 1, the schedule-correction, slices 3/4/5/6/7/8/9/10 — plus the 13-17
above): **roughly 23-27** — the same range as before, honestly, because landing slice 10 itself
was already counted inside the prior "remaining" figure as one of the summed step terms; landing
it moves one slice from "remaining" to "landed" without changing the real total — still **honestly
...
#1331, then #1333, then #1335, then #1337 — see §6's own "Revised estimate" paragraph above), so fixed `P7.N` numbers here would silently claim
