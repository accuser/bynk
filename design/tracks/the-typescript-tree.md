...
- **`verbatim_origins`** — count of distinct `VerbatimOrigin` enum variants still constructed.
  ...
  all, P7.8/#1313) — every remaining reference to the three is a `bynk-ts` unit-test fixture
  (`program.rs`/`printer.rs`, excluded from the probe's own count by its `test_mod_ranges`
  exclusion), not a live call site. Removing the three dead variants is, per §9's own precedent,
  separate, later cleanup — not this issue's scope, and not required for the probe to retire at
  its own floor: a distinct enum variant with zero production callers doesn't move
  `verbatim_origins`'s count, only a real construction site does. Full argument: `design/
  decisions/`'s ADR from #1502 (number assigned at merge).
