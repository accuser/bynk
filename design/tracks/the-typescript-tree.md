...
- **`ts_writes`** — TypeScript-producing `write!`/`writeln!`/`format!` outside `bynk-ts` **and**
  ...
  can reach either. **Both since resolved further: `pred_condition_and_message` landed for real
...
- **`verbatim_origins`** — count of distinct `VerbatimOrigin` enum variants still constructed.
  ...
  size. **Settled by #1502: the argued floor is 1, matching the live probe exactly, file-by-file
  the same way `verbatim_sites`' own floor was.** `VerbatimOrigin` has four variants
  (`bynk-ts/src/program.rs:1876`) — `Contracts`, `Secrets`, `RuntimeUse`, `NotYetConverted` — but
  only `NotYetConverted` has a real, non-test construction site anywhere in `bynk-emit/src`
  (confirmed by direct grep, not assumed): `project.rs:2480` (`adapter_bindings[name].content`)
  and `project.rs:2509` (`emitter::emit_runtime_module()`), the same two sites `verbatim_sites`'
  own floor of 2 already names as permanent (ADR 0399/#1486) — foreign, user-authored TypeScript
  and a committed npm build artifact, neither ever generated by `bynk-emit`. `Contracts`/`Secrets`/
  `RuntimeUse` are genuinely dead in production, exactly as §9's own "Risks" section already
  found (Arc C's originally-named first-slice trio turned out not to need `bynk-ts` conversion at
  all, P7.8/#1313) — no non-test, non-comment reference to any of the three exists anywhere in
  `bynk-emit/src` (the probe's own scan scope, `xtask/src/greenfield_status.rs:1841`); their only
  live references are `bynk-ts`'s own unit-test fixtures (`program.rs`/`printer.rs`, a different
  crate the probe never scans at all) and `xtask`'s own doc comments and probe unit-test fixtures
  (`greenfield_status.rs`, also out of scope). Removing the three dead variants is, per §9's own
  precedent, separate, later cleanup — not this issue's scope, and not required for the probe to
  retire at its own floor: the probe counts distinct variant *names referenced* on any non-test,
  non-comment `bynk-emit/src` line (`verbatim_origins_violations`'s own doc comment), not
  construction sites specifically — a bare qualified mention would move it exactly like a real
  `TsStmt::verbatim` call would, and no such mention exists for the three dead variants today.
  Full argument: `design/decisions/`'s ADR from #1502 (number assigned at merge).
...
**P7.d1 re-grounding (landed as P7.12, below — this section narrates the analysis as it
happened, under the provisional name, before that number was assigned) — `ir/lower.rs` is not one
crate's
...
shifting as expected once ~11,850 lines and their tests left it).

**P7.13 (landed) — R10.2 verification (finding #39: the LSP linking pure emission code it never
executes), verify-only, no code changes.** Checked `bynk-lsp`'s and `bynk-ide`'s own real
(`-e normal`, excluding dev/build-only) `cargo tree` — neither depends on `bynk-emit`, `bynk-ir`, or
`bynk-lower`, before or after P7.12's carve. `bynk-lsp/Cargo.toml`'s own comment already documents
why: "content-ownership track (#1086), slice 0: deliberately no `bynk-emit` dependency here" —
`bynk-ide` re-shapes everything the LSP needs from `bynk-check`/`bynk-project` into its own
IDE-facing surface, with no `bynk-emit` type crossing the boundary. This is the same structural fact
that closed R10.2 once already, at P4.2 (#1122, "all nine `bynk_emit::project`/`bynk_emit::emitter`
reach points repointed... `bynk-ide/Cargo.toml`'s `bynk-emit` dependency line deleted" — see
`design/bynk-greenfield-compiler.md`'s own status table). This slice's own real question — does the
*new* split change anything — is answered no: `bynk-ir`/`bynk-lower` don't leak into `bynk-lsp`'s
production tree either, confirmed by the same check. Two dev-only paths reach `bynk-emit` from
`bynk-lsp`'s test builds, both irrelevant to what a shipped language-server binary links: `bynk-lsp`
→ `bynk-testkit` (dev-dependency) → `bynk-emit` (normal), and separately `bynk-check` →
`bynk-emit` under `bynk-check/Cargo.toml`'s own `[dev-dependencies]` (the P4.1/#1115 differential
fixture) — review of this slice caught that the note's first draft named only the first path.

**Known gap, recorded rather than silently left implicit (review of this slice):** the CI-gated
probe R10.2 actually rests on, `ide_emit_edge` (`xtask/src/greenfield_status.rs:1313`), only greps
`bynk-ide/Cargo.toml` for a `bynk-emit` line — it never reads `bynk-lsp/Cargo.toml` directly (yet
finding #39 was literally about the LSP), never checks for `bynk-ir`/`bynk-lower` (the crates *this
slice's own* P7.12 gate created), and is section-unaware (a `[dev-dependencies]` line would false-
trigger it, the mirror image of the dev/production distinction just drawn above for `bynk-testkit`/
`bynk-check`). The manual `cargo tree -e normal` check above is a correct point-in-time verification,
but the standing guard does not cover what it covered — a future `bynk-lsp` → `bynk-ir` edge, say,
would ship silently. Left as a known limitation rather than extended here (extending `ide_emit_edge`
to `{bynk-ide, bynk-lsp}` × `{bynk-emit, bynk-ir, bynk-lower}`, `[dependencies]`-section-aware, is
real and mechanical but is a code change with its own tests and `design/pending` entry — out of
scope for a verify-only pass, worth its own small follow-up slice). No code changed this pass; no
`design/pending` entry, same doc-only precedent as P7.10/P7.11.

**P7.14 (landed) — R8.2/R4.10: the `__ctxBrand`/`is_uses_commons_type` predicate mirror closed,
scoped to the real, evidenced defect rather than the rule's own maximal prose.** R4.10's own
rationale names one concrete problem: `emit_context_rebrands` (`bynk-emit/src/emitter.rs`) and
`ResolvedCommons::is_uses_commons_type`/`prepare_unit_check_ctx` (`bynk-check`) each independently
...
*locally-declared* type's own branded literal — a second, distinct mechanism from `__ctxBrand`'s
*rebranding* of an *imported* commons type). Checked for a real duplication risk there too: none
found — `ctx.owning_context` is already a single, non-duplicated project-level fact with no
checker-side mirror to diverge from; the emitter reads it once and formats it, which is not the
"two independently hand-maintained copies" shape R4.10's own historical evidence (#655) is about.
R4.10's own closing line — "One `Ty` carrying its brand, one emitter reading it" — is not
implemented literally (no new field lives on `Ty`/`TypeDecl` itself): the concrete, evidenced defect
was the predicate mirror, not the absence of a stored string, and closing it with a shared function
achieves the same real guarantee (no structural drift possible) without a broader `TypedCommons`
...
genuinely permanent. **Corrected by #1471 (landed) and #1472 (landed) — see below and §6's own
Part 1 correction (#1472) above: `pred_condition_and_message` closed for real** (the two
`TsBinaryOp` variants landed, both real callers converted, `ts_writes` 860→851) **and
...
was. Full argument: `design/decisions/`'s ADR from #1462
...
**#1475 landed — `tests_emit.rs`'s 83 and `workers_entry.rs`'s 34 sites read end to end, not
sampled, closing Arc F's own long-standing caveat.** `workers_entry.rs`'s 34 sites confirm Arc F's
own "all already-argued" claim exactly: every site is either cheap identifier/message-text glue
feeding one of this file's own real node-builder helpers (`ident`/`const_`/`str_lit`/
`method_call`/…), or a raw-text argument to the already-argued cross-file `deserialise_call`/
`brand_assertion` pair (`workers.rs`'s own established precedent, #1321/#1323). No new gap found.
The one unelaborated "small fold-in candidate" Arc F's own citation never named is most plausibly
the repeated `format!("__r_{pname}")` re-derivation (independently re-built at 4-5 call sites
within both `emit_http_route_dispatch` and `emit_path_param_construction` rather than bound once)
— a real, tiny `ts_writes`-reducing tidy (fewer `format!` call sites, same output), not a
tree-conversion gap; not confirmed against Arc F's own original intent, since it was never
...
for the full bucket accounting (614 permanent, individually argued; 190 a newly-named permanent
structural category — identifier/type-name/message-text `String` construction feeding an
already-real node's leaf field; 5 real, small, tractable, named-but-not-scheduled sites)
and §6 for the six landing entries (#1481–#1486) that closed the gap between Arc F's own opening
figure and this floor.**

**#1502 (landed) settles `verbatim_origins`'s own floor at 1, matching the live probe exactly.**
`VerbatimOrigin` has four variants; only `NotYetConverted` has a real production construction
site, at exactly the same two places `verbatim_sites`' own floor already names as permanent —
`project.rs:2480`/`:2509`. `Contracts`/`Secrets`/`RuntimeUse` are dead in production (only
`bynk-ts` unit-test fixtures still reference them), consistent with §9's own "Risks" finding;
removing them stays separate, later cleanup, per that same section, not required for this probe
to retire at its own floor. See §5 for the full argument.

**All three probes now read their own argued floor: `ts_writes` 809 (#1501), `verbatim_origins`
1 (#1502), `verbatim_sites` 2 (#1486/ADR 0399).** §12's own three-probe bar is met. What remains
is executing this section's own retirement procedure — remove this doc, append its closing
summary to `../archive/retired-tracks.md`, close #1293 — tracked as #1503.
