...
**Revised estimate, corrected five times now — by review of #1332 (the arithmetic), by #1333's
own step (1) closure (the real per-step sizing), by #1335's own step (2) split, by #1337's
own step (3) closure, and now by #1339's own step (2) closure.** Step (2) is now **fully landed**
(#1339 closed the deferred dispatcher plus the 3 object-builders #1335 left open — no longer
gated on the `TsObjectEntry::Raw` decision, #1337 obviated it, see above) and, like step (3)
before it, drops out of the remaining-work sum entirely. Summing the list directly: steps
(4)(5)(6)(8)(10) are one slice each = 5 (steps (2) and (3) both removed, now landed); step (7) is
"1-2"; step (9) is "3-5"; step (11) (the ICU cluster) is "1-2". Floor: 5 + 1 + 3 + 1 = **10**;
ceiling: 5 + 2 + 5 + 2 = **14** — `emit.rs`'s own remaining tree is now **roughly 10-14 slices**
(down from 11-15, since step (2) is done). Total remaining from here: `emit.rs` (10-14) + the
`tests_emit.rs` pair (2) — #1331/#1332, slice 8 (#1333), slice 9 (#1335), slice 10 (#1337), and
slice 11 (#1339) are all already landed, no longer "remaining" — **roughly 12-16 more slices from
here**. Arc C's own real total (the **11** slices already landed — slice 1, the schedule-
correction, slices 3/4/5/6/7/8/9/10/11 — plus the 12-16 above): **roughly 23-27** — the same range
as before, honestly, because landing slice 11 itself was already counted inside the prior
"remaining" figure as one of the summed step terms; landing it moves one slice from "remaining" to
"landed" without changing the real total — still **honestly no longer close to the original
pre-Arc-C ~19 estimate**, stated plainly, not softened, the same discipline every correction in
this section has applied. `emit_agent`'s own sub-decomposition (step 9) remains the single
largest source of variance in this range; the ICU cluster (step 11) and step (7) are smaller,
secondary sources.
...
#1331, then #1333, then #1335, then #1337, then #1339 — see §6's own "Revised estimate" paragraph above), so fixed `P7.N` numbers here would silently claim
