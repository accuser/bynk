...
**Arc D — settling (~8 slices; 4 landed as P7.10/P7.11/P7.12/P7.13, 4 remaining)**
...
`design/bynk-greenfield-compiler.md`'s own status table). P7.d7's own real question — does the
*new* split change anything — is answered no: `bynk-ir`/`bynk-lower` don't leak into `bynk-lsp`'s
production tree either, confirmed by the same check. (`bynk-lsp` does depend on `bynk-testkit` as a
dev-dependency, and `bynk-testkit` depends on `bynk-emit` for its own cross-crate test fixtures —
not a production link, and not what R10.2/finding #39 are about: a shipped language-server binary
never links its own dev-dependencies.) No code changed; no `design/pending` entry, same doc-only
precedent as P7.10/P7.11.
