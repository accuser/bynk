...
were scoped separately and are now landed/excluded per the correction above); (3) the
...
**Revised estimate, corrected twice now — first by review of #1332 (the arithmetic), then by
#1333's own step (1) closure (the real per-step sizing).** Summing the list directly with step (1)
now closed and step (11) named: steps (2)(3)(4)(5)(6)(8)(10) are one slice each = 7; step (1) is
**0** (closed by #1333, landed); step (7) is "1-2"; step (9) is "3-5"; step (11) (the ICU cluster,
newly named) is "1-2", comparable in scope to `events_fanout.rs`/`workers_entry.rs`'s own `Switch`/
`Conditional` work. Floor: 7 + 0 + 1 + 3 + 1 = **12**; ceiling: 7 + 0 + 2 + 5 + 2 = **16** —
`emit.rs`'s own remaining tree is **roughly 12-16 slices**, down from the pre-#1333 14-18 (step
(1)'s own closure removes 3-4 from the floor and ceiling; step (11)'s own naming adds back 1-2).
Total remaining from here: `emit.rs` (12-16) + the `tests_emit.rs` pair (2) — this design increment
(#1331/#1332) and slice 8 (#1333) are both already landed, no longer "remaining" — **roughly 14-18
more slices from here**. Arc C's own real total (the 8 slices already landed — slice 1, the
schedule-correction, slices 3/4/5/6/7/8 — plus the 14-18 above): **roughly 22-26**, revised down
slightly from the design pass's own 24-28 but still **honestly no longer close to the original
pre-Arc-C ~19 estimate** — stated plainly, not softened, the same discipline every correction in
this section has applied. `emit_agent`'s own sub-decomposition (step 9) remains the single largest
