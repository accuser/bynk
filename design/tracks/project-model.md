...
to avoid the new entry point's seven documented residual-gap categories (named on `analyse_project`'s own doc
...
**Post-merge review correction (#1117).** The review that landed after #1117 merged found test/integration-suite
processing (`process_tests`/`process_integration_tests`) mis-filed as "orthogonal" rather than as a seventh
category — those two functions run unconditionally in `Mode::Analyse` too and push into the shared error sink,
so the new entry point silently drops any diagnostic originating in a `suite`/`test integration` body, and
(since both take `&mut RefSink`) silently omits every binding edge inside a test file from `index` as well. The
doc comment now names this as residual-gap category 7, and a third differential fixture
(`new_entry_point_omits_test_body_diagnostics`) pins the divergence directly rather than the fixture merely
avoiding it by construction.
