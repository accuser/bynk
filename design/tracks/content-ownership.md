...
- **Realises:** R2.3 (`../bynk-greenfield-compiler.md`) — *"no ambient
  filesystem or global state; `Sources` is constructed once, at the process
  edge, and is the compiler's only view of file contents"* — specifically the
  `bynk-ide` row of its `fs_below_driver` table
  (`../bynk-greenfield-compiler.md:2515`), open since T0.7 (#1006/#1012).
- **Posture:** Feature track per
  [ADR 0076](../decisions/0076-feature-track-posture.md). Qualifies on two
  axes (§2): it is multi-increment (production-code seam, then a 150+-site
...
- **The unusual starting position — five investigations, one shared
  ...
  `symbols.rs` alone, because ~130 test call sites across `bynk-ide`,
...
Three call sites read `.bynk` source content straight from disk, bypassing
...
- **Multi-increment.** The production-code half (§4, slices 0–3: a new seam
  ...
  convention across ~130 call sites, then deleting the fallback) are each too
...
- **Surface not yet settled.** Three genuinely open questions (§3): the shape
  of the new `bynk-ide`-exposed seam (`bynk-lsp` deliberately does not depend
  on `bynk-emit`, and `bynk-ide` is itself one of the three
  `fs_below_driver`-probed crates, so the walk can't simply relocate into
  either); whether R2.3's "no ambient filesystem" also bans directory
  *enumeration*, not just content reads; and what replaces a 150+-site test
  convention without turning every test file into boilerplate.
...
current counts (§7 of the investigation this doc is built from): 75 bare
`diagnose_project` calls plus 10 `diagnose_project_with` calls (85 total,
**all test-only**, none in production) across `bynk-ide`'s inline
`#[cfg(test)]` modules, `bynk-lsp/tests`, and `bynkc/tests`; separately, 52
`CompileOptions::single`/`::split` call sites across 39 files in
`bynkc/tests`/`bynk/tests`. Deleting `discovery.rs`'s fallback means every one
of those ~137 sites must supply a *complete* sources map instead of relying on
it silently filling gaps.
...
not a per-test rewrite — or 137 call sites will not migrate cleanly and this
...
- **Slice 5 — migrate the remaining ~130 test call sites** to the slice-4
  helper. §3.4 decides whether this is one slice or several.
- **Slice 6 — delete `discovery.rs`'s content fallback**, and its enumeration
  walk too if §3.2 resolves broad. `fs_below_driver` reaches 0 for
  `bynk-emit`; combined with slice 2, the probe reads 0 end to end (only
  `bynk-fmt`'s already-0 count remains in the table).
...
- **§3.3 — the test-harness replacement convention.** ~137 call sites will
...
- [ ] Slice 5 — migrate the remaining ~130 test call sites
- [ ] Slice 6 — delete `discovery.rs`'s content fallback

## 8. Done when

`cargo xtask greenfield-status`'s `fs_below_driver` reads 0 for `bynk-emit`
and `bynk-ide` (`bynk-fmt` is already 0), `bynk-emit/src/project/discovery.rs`
has no content-reading disk fallback left (and no enumeration walk either, if
§3.2 resolves broad), and a behaviour-driven test — mirroring ADR 0202's
