...
- **Status:** **COMPLETE — all slices shipped (0–3); ready to retire.** The contention this
  track exists to remove is gone: a feature PR lands a `design/pending/` file with no numbers,
  and the per-merge stamp workflow assigns the version + ADR number on `main` (Slices 0–2,
  v0.186, ADR 0206). The deferrable surface-shrink (Slice 3, #693) landed too — `llms-full.txt`
  is now a build artifact. A retirement PR (remove this doc, append the closing summary to
  `../archive/retired-tracks.md`, `Closes #685`) is the only remaining step. The spine is
...
- **Slice 3 — shrink the surface.** `site/public/llms-full.txt` becomes a build artifact —
  regenerated by the site's `prebuild` npm hook on every `npm run build` (CI + deploy), no
  longer committed; the two `--check` guards and the `bump-version.sh` regeneration retire.
  The 12 crate README `[dependencies]` examples stay committed (they are published to
  crates.io) and the 7 Book banners stay committed (an Astro build-time injection was judged
  not to earn its keep once the stamp removed the contention), so this is the llms-full slice
  only. Infra-only: no bump, no ADR. **Shipped — #693.**
