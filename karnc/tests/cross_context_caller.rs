...
//! analogue). Skips loudly without a toolchain; `KARN_REQUIRE_TSC=1` turns the
//! skip into a failure.
...
fn cross_context_caller_reads_live_id_and_fails_closed() {
    ...
    let tmp = std::env::temp_dir().join(format!("karn-xctx-caller-{}", std::process::id()));
    ...
    let out = match karnc::compile_project(
