...
//! see phase 7's own closing summary in `design/archive/retired-tracks.md`).
//!
...
//! floor (`ast_importers` = 5, `emit_abi_shapes` = 1). `ts_writes`/`ts_any` are the same
//! shape: they converged toward an argued floor over dozens of slices, the same
//! trajectory `ast_importers` had throughout phase 6's 59 — retired at `ts_writes` =
//! 809, `ts_any` = 26 (phase 7's own closing summary, `design/archive/retired-tracks.md`),
//! neither the literal 0 first proposed — gated throughout despite the churn that
//! implies, a deliberate call argued in
...
/// confirmed unchanged by the #1486 capstone) — `project.rs:2478`/`:2507`,
/// an adapter binding's own foreign, user-authored TypeScript and a
/// committed npm build artifact, neither ever generated by `bynk-emit`; full
/// accounting in phase 7's own closing summary, `design/archive/
/// retired-tracks.md`. Read **0** at this slice's own landing, same reason
