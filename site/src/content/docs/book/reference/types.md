...
generic value instead. A refined or opaque body cannot be generic (its base is a
fixed primitive); only a record or **sum** body may be
(`bynk.generics.generic_non_record`). Generic sums are described under
[Sum types](#sum-types).
