...
pub fn emit(commons: &TypedCommons) -> String {
    ...
    // v0.153 (ADR 0177): a commons body that references the built-in
    // `HttpResult` — its type (`HttpResult<T>`) or its value namespace
    // (`HttpResult.Ok`, `HttpResult.NotFound`, …) — imports it. This reaches
    // beyond the `?`-Option lift: any commons `fn` returning `HttpResult`
    // needs the import.
    let uses_http = body.contains("HttpResult<") || body.contains("HttpResult.");
