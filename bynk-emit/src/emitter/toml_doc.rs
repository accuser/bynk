...
impl TomlDocument {
    ...
    /// tree-level equivalent of `emitter::wrangler::set_wrangler_main`'s own
    /// text-based patch (which parses an *existing on-disk* `wrangler.toml`
    /// via `toml_edit`, for callers with only text). A caller that already
    /// holds the real tree (`bynk-strip::strip_project_to_js`, P7.6 #1309
    /// Decision E) uses this instead, so patching the JS artefact's entry
    /// module never prints then re-parses just to change one field.
    /// A no-op if the document has no root `main` entry.
    pub fn set_main(&mut self, value: impl Into<String>) {
...
mod tests {
    ...
    #[test]
    fn set_main_is_a_no_op_without_a_root_main_entry() {
        let mut doc = TomlDocument::new(
            "Generated by bynkc — do not edit by hand.",
